...
fun PreferencesScreen(modifier: Modifier = Modifier) {
    ...
    Column(
        ...
        Button(
            onClick = {
//                SharedPreferenceManager.saveValue(context, key, input)
                scope.launch {
                    DataStoreManager.saveValue(context, key, input)
                }
            }
        ) {
            Text("pref 저장")
        ...
        Button(
            onClick = {
//                SharedPreferenceManager.deleteValue(context, key)
                scope.launch {
        ...
        Button(
            onClick = {
//                result = SharedPreferenceManager.getValue(context, key) ?: "없음"
                scope.launch {
                    result = DataStoreManager.getValue(context, key) ?: "없음"
                }
            }
        ) {
            Text("pref 조회")
